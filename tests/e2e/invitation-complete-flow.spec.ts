...
test.describe("Complete Invitation Flow - E2E", () => {
	test("should complete full invitation acceptance flow", async ({ page }) => {
		...
		const participantsInfo = page.locator(
			"text=/\\d+ (of \\d+ )?participants/",
		);
	...
	test("should handle expired invitation gracefully", async ({ page }) => {
		...
		await expect(
			page.locator("text=Unable to Access Discussion"),
		).toBeVisible();
		...
		const helpText = page.locator(
			"text=Invitation links have an expiration date",
		);
	...
	test("should handle full discussion scenario", async ({ page }) => {
		...
		await expect(
			page.locator("text=maximum number of participants"),
		).toBeVisible();
	...
	test("should handle concurrent users accessing same invitation", async ({
		browser,
	}) => {
	...
	test("should preserve form data during page interactions", async ({
		page,
	}) => {
		...
		const invitationCard = page
			.locator('[role="main"], .container, div')
			.first();
